...
class Pagie1():
    ...
    def read_dataset(self):
        ...
        train = []
        for x in drange(-5,5.4,0.4):
            for y in drange(-5,5.4,0.4):
                z = pagiepolynomial(x,y)
                train.append([x,y,z])

        #function = eval(self.function)
        #y_train = map(keijzer9,train_range)
        self.__train_set = train
        self.__number_of_variables = 2
        self.training_set_size = len(self.__train_set)

        test = []
        for xx in drange(-5,5.0,.1):
            for yy in drange(-5,5.0,.1):
        ...
        #y_test = map(keijzer9,test_range)
...
if __name__ == "__main__":
    import sge
    sge.setup("parameters/pagie1.yml")
    eval_func = Pagie1(params['RUN'])
