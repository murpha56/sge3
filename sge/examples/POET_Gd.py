...
#        self.calculate_rrse_denominators()
    ...
    def read_dataset(self):
        ...
        #with open('resources/SymRegPOET/Gd_Epoch1_Train', 'r') as dataset_file:
        #    for line in dataset_file:
        #        dataset.append([float(value.strip(" ")) for value in line.split(" ") if value != ""])

        with open('resources/SymRegPOET/Gd_Epoch1_Train', 'r') as dataset_file:
            lines = dataset_file.readlines()
            # Take 10% of the lines randomly
            sampled_lines = random.sample(lines, max(1, len(lines) // 2))  # Ensure at least one line is taken
            for line in sampled_lines:
                dataset.append([float(value.strip(" ")) for value in line.split(" ") if value != ""])

        with open('resources/SymRegPOET/Gd_Epoch1_Test_Motif', 'r') as test_file1:
            for line in test_file1:
                test1.append([float(value.strip(" ")) for value in line.split(" ") if value != ""])
        with open('resources/SymRegPOET/Gd_Epoch1_Test_Regex', 'r') as test_file2:
